...
fFindWater<-function(layers, df, what=c("ATSW", "TTSW", "FTSW", "WL", "WLUL", "WLST"), weightedbyroots=FALSE){
  ...
  if (what=="ATSW") {
    ...
    result<-(pmax(WL-WLLL,0)/(WLUL-WLLL))*weights
...
fNFixationDuringSeedGrowth <- function(cDailySeedWeightIncrease, pGrainMaxConcentrationN,pSNCG, cGrowthLAI, pSpecLeafNGreenLeaf, pNUPmax, sCoefBiologicalNFixation,sAccumulatedVegetativeDryMatter,cCoefWaterStressSaturation,cDailyStemWeightIncrease,cDryMatterProduction,pGrainConversionCoefficient,cFTSWRrootZone,sTotalAvailableUptakeN,sAccumulatedStemDryMatter,pGrainMinConcentrationN,sLAI,pStemMinimumNconcentration, fxlf){
  ...
  # Moreover, the book asserts that GNC is said to be constand during seed growth (Sinclair and Soltani Model)
  ...
  sDailyRateNFromStem[NLeafGreaterThanNeeded] <-(pmin(sDailyAccumulationLeavesN - NUP2, sAccumulatedNStem - sAccumulatedStemDryMatter*pStemMinimumNconcentration ))[NLeafGreaterThanNeeded]
  ...
  sDailyRateNFromStem[NLeafSmallerThanNeeded] <- 0 ### Not necessary, just to be clear
...
rUpdatePlantNitrogen <-function(){
  ...
  if(any(crop_presence)){
    ...
    pSNCG <- ALLCROPS[crops, "pSNCG"][crop_presence] # to get pSNCG 
    ...
    ## Listing of variables needed in the procedure to calculate cumulative variables
    ...
    ## Calculation of intermediate variables for the 2 main functions, need to be calculated to calculate FXLF
    ...
    # TRLN is the Total mobilizable N available in the plant  and is used in the 2nd function
    ...
    # FXLF is proportion of the daily N transfer from the leaves, need to be calculated during Seed Growth
    
    
    # Initialization of cDemandNAccumulation (NUP)
    cDemandNAccumulation <- rep(0,nrow(ALLDAYDATA))
    ...
    FixationVegetativeGrowth <- data.frame(INST <- rep(0,nrow(ALLDAYDATA)), XNST <- rep(0,nrow(ALLDAYDATA)), INLF <- rep(0,nrow(ALLDAYDATA)), XNLF <- rep(0,nrow(ALLDAYDATA)), BNF <- rep(0,nrow(ALLDAYDATA)), INGRN <- rep(0,nrow(ALLDAYDATA)), NUP <- rep(0,nrow(ALLDAYDATA)))
    ...
    # The function returns INST, XNST, XNLF, INLF, NUP, BNF and INGRN during vegetative growth (7 numeric vectors)
    
    cDemandNAccumulation[VegetativeGrowthFilter] <- (FixationVegetativeGrowth$NUP)[VegetativeGrowthFilter]
    # NUP during the Vegetative Growth, calculated by extraction of the 7th row of the dataframe returned by the function fNFixationDuringVegetativeGrowth
    ...
    # No information about the mean of this variable (CUMBNF)
    ...
    # Accumulated Nitrogen in the stem during Vegetative Growth, updated by adding the daily accumulation of nitrogen in the stem and subtracting the daily rate of nitrogen mobilized from stems
    ...
    # Accumulated Nitrogen in the leaves Vegetative growth, updated by adding the daily accumulation of nitrogen in the leaves and subtracting the daily rate of nitrogen mobilized from the leaves
    ...
    # Accumulated Nitrogen in the grain during seed growth, updated by adding the daily accumulation of nitrogen in grain (Not necessary, there is no increase of nitrogen in the grain during Vegetative Growth)
    ...
    # Total Nitrogen accumulated during Vegetative Growth (Stem + Leaves)
    }
    ...
    FixationSeedGrowth <- data.frame(INST <- rep(0,nrow(ALLDAYDATA)), XNST <- rep(0,nrow(ALLDAYDATA)), INLF <- rep(0,nrow(ALLDAYDATA)), XNLF <- rep(0,nrow(ALLDAYDATA)), BNF <- rep(0,nrow(ALLDAYDATA)),INGRN <- rep(0,nrow(ALLDAYDATA)),NUP <- rep(0,nrow(ALLDAYDATA)))
    ...
    # The function returns INST, XNST, XNLF, INLF, NUP, BNF and INGRN during seed growth (7 numeric vectors)
    
    
    cDemandNAccumulation[SeedGrowthFilter] <- (FixationSeedGrowth$NUP)[SeedGrowthFilter]
    # NUP during the seed growth, calculated by extraction of the 7th row of the dataframe returned by the function fNFixationDuringSeedGrowth
    ...
    # Accumulated Nitrogen in the stem during seed growth, updated by adding the daily accumulation of nitrogen in the stem and subtracting the daily rate of nitrogen mobilized from stems
    ...
    # Accumulated Nitrogen in the leaves during seed growth, updated by adding the daily accumulation of nitrogen in the leaves and subtracting the daily rate of nitrogen mobilized from the leaves
    sAccumulatedNGrain[SeedGrowthFilter] <- sAccumulatedNGrain[SeedGrowthFilter] + (FixationSeedGrowth$INGRN)[SeedGrowthFilter]
    # Accumulated Nitrogen in the grain during seed growth, updated by adding the daily accumulation of nitrogen in grain
    sTotalAccumulatedNitrogen[SeedGrowthFilter] <- sAccumulatedNStem[SeedGrowthFilter] + sAccumulatedLeafNitrogen[SeedGrowthFilter]
    # Total Nitrogen accumulated during seed growth (Stem + Leaves)
    
    }
    
    # Add new values to ALLDAYDATA
